...
// Aseguramos que la variable $busqueda esté siempre definida
$busqueda = isset($_GET['busqueda']) ? $_GET['busqueda'] : '';  // Si no hay búsqueda, asignamos una cadena vacía

// Filtrar los documentos de Odontología (código sin "A")
$query = "SELECT d.*, u.nombres, u.apellidos, c.nombre AS categoria, cri.nombre AS criterio, t.nombre AS tipo_area
          ...
          WHERE d.codigo_documento NOT LIKE '%A%'";  // Filtrar solo los documentos sin "A" en el código

// Comprobamos si hay una búsqueda y modificamos la consulta en consecuencia
if (!empty($busqueda)) {
    $likeBusqueda = '%' . $busqueda . '%';
    $query .= " AND d.titulo LIKE ?";  // Agregamos el filtro para el título
...
<!-- Buscador -->
<form method="GET" action="vistaodo.php" autocomplete="off">
    <input type="text" id="busqueda" name="busqueda" placeholder="Buscar por título..." oninput="mostrarSugerencias(this.value)" value="<?php echo htmlspecialchars($busqueda); ?>" onkeydown="if(event.key === 'Enter') { this.form.submit(); }">
    <div id="sugerencias" class="sugerencias"></div>
</form>

<!-- Mostrar solo los documentos que coinciden con la búsqueda -->
